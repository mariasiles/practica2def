...
//Punt extra
//Definim la funció que permetrà obtenir una mesura relativa de l'error de l'aproximació de la solució del sistema d'equacions lineals
float NormaRelativaResidu(float M[N][N], float vect[N], float vector[N]) {
	    float residu[N]; //Declarem el vector que guardarà el residu del sistema d'equacions lineals Ax=b
            float norma_residu = 0.0; //Declarem la variable que guardarà la suma dels quadrats dels elements del vector residu
	    float norma_b = 0.0; //Declarem la variable per acumular la suma dels quadrats dels elements del vector b
	      for (int i = 0; i < N; i++) {
		      residu[i] = 0.0;
		      for (int j = 0; j < N; j++) {
			      residu[i] += M[i][j] * vect[j]; //Multiplica la matriu per el vector i actualiza el resultat
		      }
		      residu[i] -= vector[i];//Li restem el costat dret de l'equacio(vector b)
	      }
	      for (int i = 0; i < N; i++) {
		      norma_residu += residu[i] * residu[i]; //calculem els quadrats dels elements vector residu
		      norma_b += vector[i] * vector[i];  //calculem els quadrats dels elements  del vector b
	      }
